...
// If you choose to do Task 1.b, complete the TODOs marked in this file
...
// TODO for 1.b: Write your compare and swap functions here!
...
int main() {
  ...
  gsort(A, 44, sizeof(char), &compare_char, &swap_char);
  printf("The array A, initial part sorted:\n%s\n\n", A);
  gsort(A, 57, sizeof(char), &compare_char, &swap_char);
  printf("The array A, sorted all the way to the NUL terminator:\n%s\n\n", A);

  printf("The array B: ");
  print_ints(B, 10);

  gsort(B, 3, sizeof(int), &compare_int, &swap_int);
  printf("The array B, first 3 elements sorted: ");
  print_ints(B, 10);

  gsort(B, 10, sizeof(int), &compare_int, &swap_int);
  printf("The array B, all elements sorted: ");
  print_ints(B, 10);

  printf("passed!\n");

  // TODO for 1.b: Remove this line!
  return 0;
  ...
  // TODO for 1.b: Sort by frequency
  ...
  bool sorted = true;
  for(int i = 0; i < 13; i++) {
    sorted = sorted && (C[i].count <= C[i+1].count);
  }
  if(sorted)  {
    printf("This is correctly sorted by frequency.\n");
  }
  else{
    printf("This is not sorted.\n");
  }

  // TODO for 1.b: Sort by word
  ...
  sorted = true;
  for(int i = 0; i < 13; i++) {
    sorted = sorted && (0 >= strcmp(C[i].word,C[i+1].word));
  }
  if(sorted)  {
    printf("This is correctly sorted by string.\n");
  }
  else{
    printf("This is not sorted\n");
  }
