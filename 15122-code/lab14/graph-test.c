...
int main() {
  ...
  printf("%s\n", dfs(maze15c, 7, 12) ? pass : fail);

  printf("Searching connected15 with BFS for a path from 7 to 12...\n");
  printf("%s\n", bfs(maze15c, 7, 12) ? pass : fail);

  assert(!connected(maze6));
  printf("\nNOT CONNECTED\n\n");
  assert(!connected(maze15d));
  printf("\nNOT CONNECTED\n\n");
  assert(connected(maze15c));
  printf("\nCONNECTED\n\n");

  graph_t maze5d = disconnected5();
  graph_t maze5c = connected5();

  assert(!connected(maze5d));
  printf("\nNOT CONNECTED\n\n");
  assert(connected(maze5c));
  printf("\nCONNECTED\n\n");
